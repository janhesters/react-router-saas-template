...
import { getAcceptedEmailInviteOrganizationSlug } from "~/features/organizations/accept-email-invite/accept-email-invite-helpers.server";
...
export async function onboardingUserAccountAction({
  ...
  const inviteOrganizationSlug =
    acceptedEmailInviteOrganizationSlug ?? inviteLinkInfo?.organizationSlug;
  const inviteMembership = inviteOrganizationSlug
    ? user.memberships.find(
        ({ organization }) => organization.slug === inviteOrganizationSlug,
      )
    : undefined;

  if (inviteMembership) {
    const i18n = getInstance(context);
    const { organization } = inviteMembership;
  ...
  if (user.memberships.length > 0) {
    ...
    return onlyMembership
      ? redirect(
          href("/organizations/:organizationSlug/dashboard", {
            organizationSlug: onlyMembership.organization.slug,
          }),
          { headers },
        )
      : redirect(href("/organizations"), { headers });
  }
